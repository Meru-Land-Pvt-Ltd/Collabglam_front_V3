...
export default function RaiseIssueModal({
  ...
  useEffect(() => {
    ...
    if (!id) {
      setError("Missing influencer id. Please log in again as an influencer.");
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await postFormData<{ disputeId?: string }>(
        "/dispute/influencer/create",
        form
      );

      handleClose();

      const createdDisputeId = response?.disputeId;
      router.push(
        createdDisputeId
          ? `/influencer/support-centre?disputeCreated=1&ticket=${createdDisputeId}`
          : "/influencer/support-centre?disputeCreated=1"
      );
    } catch (e: any) {
      setError(
        ...
        <div className="px-6 pb-6 pt-8 sm:px-8 sm:pb-8 sm:pt-9">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600 sm:text-lg">
                Report problems related to campaign timelines, payments,
                deliverables, contracts, or communication.
              </p>
            </div>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-slate-500">
                Select one of the campaigns you have applied to or worked on.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                readOnly
                placeholder="Select a campaign to auto-fill brand"
                className="h-12 w-full rounded-xl border border-slate-300 bg-slate-100 px-4 text-base text-slate-700 shadow-sm outline-none"
              />
            </div>
          ...
          <div className="mt-6 flex items-start gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-4 text-slate-700">
            ...
            <p className="text-sm leading-6">
              Campaign and subject are required. The brand will be auto-filled
              from your selected campaign. Attach screenshots, contracts, or
              other relevant evidence to speed up review.
            </p>
          </div>
        </div>
