...
const MailList = ({
  ...
  <div className="flex flex-col w-[15.8125rem] h-[43.75rem] rounded-[1rem] border border-[#D6D6D6] bg-white  overflow-hidden">
    {/* Panel header */}
...
export const MailCompose = ({
    ...
    return (
        <div className={`relative border border-[#D6D6D6] rounded-[1rem] flex flex-col bg-white overflow-hidden transition-all duration-200
            ${isReply
                ? 'w-full h-auto'
                : isMaximized ? 'w-full h-full mx-auto' : '!h-[43.75rem] !w-[51.375rem]'
            }`}
        >
            {/* ── Header ── */}
            {isReply ? (
...
const MailEditor = ({ onClose }: { onClose?: () => void }) => {
  ...
  return (
    <div className="flex w-11/12 border-t border-[#D6D6D6] p-[0.5rem] mx-auto my-[1.5rem] gap-[0.5rem]">
      <MailList
