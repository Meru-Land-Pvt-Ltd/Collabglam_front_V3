...
export default function DisputeDetailDrawer({
  ...
  const load = useCallback(async () => {
    if (!open || !disputeId || !brandId) return;
    setLoading(true);
    setError(null);
  ...
  const postComment = async () => {
    if (!disputeId || !brandId || (!comment.trim() && !commentFiles.length)) {
      return;
    }
    ...
    try {
      ...
      await postFormData(`/dispute/brand/${disputeId}/comment`, form);
      setComment("");
      setCommentFiles([]);
      setIsReplyOpen(false);
      await load();
