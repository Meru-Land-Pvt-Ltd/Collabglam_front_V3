...
> = {
  // ✅ error bg/border now 100% (no transparency)
  error: {
    bg: "#FCEEEC",
    border: "#FCEEEC",
    title: "#191919",
...
// ✅ Accent bar colors (error uses your CSS var exactly)
const ACCENT: Record<ToastIcon, string> = {
  error: "var(--Errors-500, #E35141)",
  warning: "#FF8751",
  success: "#28A745",
  info: "#2F80ED",
};
...
export function ToastStyles() {
  return (
    <style jsx global>{`
      ...
      /* ✅ 4px dark line at the starting (left) border */
      .emc-toast::before {
        content: "";
        position: absolute;
        left: 0;
        ...
        bottom: 0;
        width: 4px;
        background: var(--emc-accent, transparent);
...
export function toast({ icon, title, text, timer = 2500 }: ToastOptions) {
  ...
  return Swal.fire({
    ...
    width: 480,
    // ✅ extra left padding so content doesn't sit under the 4px bar
